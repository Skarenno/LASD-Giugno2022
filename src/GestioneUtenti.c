...
// Schermata Iniziale. Ritorna l'utente di cui si � effettuato l'accesso
...
Utente* RegistraUtente (Utente* ListaUtenti){
	...
	while(true){
		...
		// Se il nome utente � gi� presente si richiede un nuovo inserimento.
		if(TrovaUtente(username, ListaUtenti) != NULL){
			printf("Nome utente gi� esistente. Riprovare.\n");
			continue;
		}
		break;
	}
